...
Every running project exposes live API documentation — no configuration needed.

| Endpoint | Description |
|---|---|
| `GET /openapi.json` | OpenAPI 3.1 spec (JSON), auto-generated from TypeBox schemas |
| `GET /docs` | Interactive Scalar UI — try requests directly in the browser |

Both endpoints are public (no JWT required) and are registered before the auth middleware so they are always reachable.

### What is included in the spec

- All services registered via `registerServiceSchemas()` in their `.service.ts` file
- Every service generated by `npx feathers-baas generate service` is registered automatically
- Built-in services: `users`, `roles`, `authManagement`, `files` (if plugin-files is configured)
- Auth endpoints: `POST /authentication` (login), `DELETE /authentication` (logout), `POST /authentication/refresh`

### Registering a hand-written service

If you add a service manually (without the generator), add this to its `.service.ts`:
...
### Using the spec as an agent

When you need to understand the full shape of the API (field names, required fields, response types), fetch the live spec rather than reading source files — it is always in sync with the running code:

```bash
curl -s http://localhost:3030/openapi.json | jq '.paths | keys'        # list all routes
curl -s http://localhost:3030/openapi.json | jq '.components.schemas'  # list all schemas
```

This is more reliable than reading `.schema.ts` files when services may have been added by plugins or by code you have not read yet.
