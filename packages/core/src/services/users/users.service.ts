...
import { UsersClass, UsersMongoClass } from './users.class.js'
...
export function configureUsers(app: Application): void {
  ...
  if (mongoDb) {
    app.use(
      'users',
      new UsersMongoClass({ Model: mongoDb.collection('users'), paginate }, app),
      { methods, events: [] },
    )
  } else {
    ...
    app.use(
      'users',
      new UsersClass({ Model: knex, name: 'users', paginate }, app),
      { methods, events: [] },
    )
  }
