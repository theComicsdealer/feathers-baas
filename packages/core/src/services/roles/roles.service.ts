...
import { RolesClass, RolesMongoClass } from './roles.class.js'
...
export function configureRoles(app: Application): void {
  ...
  const paginate = { default: 50, max: 200 }

  if (mongoDb) {
    app.use(
      'roles',
      new RolesMongoClass({ Model: mongoDb.collection('roles'), paginate }, app),
      { methods, events: [] },
    )
  } else {
    ...
    app.use(
      'roles',
      new RolesClass({ Model: knex, name: 'roles', paginate }, app),
      { methods, events: [] },
    )
  }
