...
> **M5 complete** — all milestones shipped. Plugin system + publish pipeline ready.
...
| M5 — Plugin system & publish | ✅ Done | Plugin interface · auto-discovery · dual ESM/CJS · changesets |
...
feathers-baas auto-discovers and installs plugins at startup. No manual wiring needed.

### Writing a plugin

A plugin is an npm package that exports a `FeathersBaasPlugin` object:

```ts
import type { FeathersBaasPlugin } from '@feathers-baas/core/exports'

const myPlugin: FeathersBaasPlugin = {
  name: 'my-plugin',
  version: '1.0.0',
  async install(app, options) {
    // Register services, hooks, middleware, etc.
...
Plugins are discovered automatically from `package.json` dependencies. A package is recognized as a plugin if either:

1. Its `package.json` contains `"feathers-baas-plugin": true`, or
2. Its `keywords` array includes `"feathers-baas-plugin"`

```json
{
  "name": "my-feathers-baas-plugin",
  "feathers-baas-plugin": true,
  "keywords": ["feathers-baas-plugin"]
}
```

At startup, `installPlugins()` scans dependencies, dynamically imports each plugin, and calls `install(app, options)`. Plugin installation happens after notifications (step 11) and before the global error hook.
...
This monorepo uses [@changesets/cli](https://github.com/changesets/changesets) for versioning and publishing.
...
All packages have `"publishConfig": { "access": "public" }`. Library packages (`core`, `plugin-files`, `plugin-notifications`) ship dual ESM + CJS builds. The CLI (`feathers-baas`) is ESM-only.

| Package | npm name | Format |
|---|---|---|
| `packages/core` | `@feathers-baas/core` | ESM + CJS |
| `packages/plugin-files` | `@feathers-baas/plugin-files` | ESM + CJS |
| `packages/plugin-notifications` | `@feathers-baas/plugin-notifications` | ESM + CJS |
| `packages/cli` | `feathers-baas` | ESM |
